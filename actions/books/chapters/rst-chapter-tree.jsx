...
export async function updateChapterOrder(bookSlug, chaptersData) {
    try {
        ...
        const existingChapters = await db.query.chapters.findMany({
            where: (chapters, { and, eq, inArray }) => and(eq(chapters.bookId, book.id), inArray(chapters.id, chapterIds)),
            columns: {
                id: true
            }
        });
