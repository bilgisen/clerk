...
/**
 * Checks if a node position is valid within the editor document
 * @param editor - The Tiptap editor instance
 * @param pos - The position to validate
 * @returns boolean indicating if the position is valid
 */
export function isValidPosition(editor: Editor | null, pos: number): boolean {
  if (!editor?.state?.doc) return false
  try {
    // Check if position is within document bounds
    return pos >= 0 && pos <= editor.state.doc.content.size
  } catch {
    return false
  }
}
...
/**
 * Checks if the current node selection is of a specific type
 * @param editor - The Tiptap editor instance
 * @param type - The node type name to check against
 * @returns boolean indicating if the current node matches the type
 */
...
/**
 * Checks if a Tiptap extension is available in the editor
 * @param editor - The Tiptap editor instance
...
export function isExtensionAvailable(
  ...
  return editor.extensionManager.extensions.some(
    (ext: AnyExtension) => ext.name === extensionName
  )
}

/**
 * Parses shortcut key combinations into a human-readable format
 * @param keys - The shortcut keys (string, array, or undefined)
 * @returns Formatted shortcut string (e.g., "⌘B" on Mac, "Ctrl+B" on Windows)
 */
export function parseShortcutKeys(keys: string | string[] | undefined | null): string {
  if (!keys) return ""
  
  // Convert to array if it's a string
  const keysArray = Array.isArray(keys) ? keys : 
                   typeof keys === 'string' ? keys.split('-') : []
    ...
    .map(key => {
      ...
      switch (key) {
        ...
        case ' ':
          return 'Space'
        default:
          // Return the key in uppercase if it's a single character
