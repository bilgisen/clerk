...
export function BookInfoForm({ onSubmit, defaultValues, isSubmitting = false, }) {
    // Initialize form with type-safe defaults
    const formMethods = useForm({
        resolver: zodResolver(bookSchema), // Type assertion needed due to complex type inference
        defaultValues: {
            title: '',
            slug: '',
            author: '',
    ...
    return (<form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6" aria-busy={isSubmitting}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="mb-1">
              ...
              <input type="text" {...register("title")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="Book title"/>
              {errors.title && <p className="text-red-500 text-sm mt-1">{errors.title.message}</p>}
            </div>
            <div className="mt-3">
              <label className="block text-sm font-medium text-muted-foreground/50">Subtitle</label>
              <input type="text" {...register("subtitle")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="Optional subtitle"/>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" {...register("author")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="Author name"/>
              {errors.author && <p className="text-red-500 text-sm mt-1">{errors.author.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground/50">Publisher</label>
              <input type="text" {...register("publisher")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="Publisher name"/>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="url" {...register("publisherWebsite")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="https://example.com"/>
            ...
            <div>
              ...
              <input type="text" {...register("isbn")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="10 or 13 digit ISBN"/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-2">
                <input type="text" {...register("series")} className="flex-1 border px-3 py-2 rounded text-md mt-1 bg-background text-foreground" placeholder="Series name"/>
                <div className="w-20">
                  <label className="block text-sm font-medium text-muted-foreground/50 text-center">#</label>
                  <input type="number" min="1" {...register("seriesIndex")} className="w-full border px-3 py-2 rounded text-md mt-1 text-center" placeholder="1"/>
                </div>
              </div>
            </div>
            ...
            return (<div>
                      <Select onValueChange={(val) => field.onChange(val)} value={value}>
                    ...
                    'CHILDREN', 'YOUNG_ADULT', 'COOKBOOK', 'TRAVEL', 'HUMOR', 'POETRY',
                    'BUSINESS', 'TECHNOLOGY', 'SCIENCE', 'PHILOSOPHY', 'RELIGION', 'OTHER'
                ].map((genre) => (<SelectItem key={genre} value={genre}>
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap gap-2 border rounded p-2 min-h-[42px]">
              {tags?.map((tag) => (<span key={tag} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                  {tag}
                  <button type="button" onClick={() => removeTag(tag)} className="ml-1.5 inline-flex items-center justify-center h-4 w-4 rounded-full text-blue-400 hover:bg-blue-200 hover:text-blue-500 focus:outline-none">
              ...
              <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={addTag} className="flex-1 border-0 p-0 text-sm focus:ring-0" placeholder="Add a tag and press Enter"/>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea {...register("description")} rows={5} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder="A brief description of the book"/>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <Controller name="coverImageUrl" control={control} render={({ field }) => (<ImageUploadField value={field.value || ''} onChange={(url) => field.onChange(url)}/>)}/>
    ...
    {/* Language */}
          <div>
            ...
            <select id="language" {...register("language")} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md">
              {languageOptions.map((lang) => (<option key={lang.value} value={lang.value}>
                  {lang.label}
                </option>))}
            </select>
          </div>
          ...
          <div>
            ...
            <input type="number" min="1000" max={new Date().getFullYear() + 1} {...register("publishYear")} className="w-full border px-3 py-2 rounded text-md mt-1" placeholder={new Date().getFullYear().toString()}/>
            {errors.publishYear && <p className="text-red-500 text-sm mt-1">{errors.publishYear.message}</p>}
          </div>

          {/* Submit Button */}
          <div className="pt-4">
            <button type="submit" disabled={isSubmitting || loading} className={`px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${(isSubmitting || loading) ? 'opacity-50 cursor-not-allowed' : ''}`}>
