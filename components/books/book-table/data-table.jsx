...
export function DataTable({ columns, data, isLoading = false, emptyState, }) {
    ...
    return (<div className="space-y-4">
      <DataTableToolbar table={table} filterColumn={columns.find(col => col.id)?.id || 'title'}/>
            ...
            {table.getRowModel().rows?.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                ...
                <TableCell colSpan={columns.length} className="h-24 text-center p-0">
                  {isLoading ? (<div className="flex items-center justify-center p-8">
                    ...
                    </div>) : emptyState ? (emptyState) : (<div className="flex flex-col items-center justify-center p-8 text-muted-foreground">
